...
function luanti_backups.generate_hash_path(timestamp)
	...
	local current = luanti_backups.world_path .. "/backups"
...
function luanti_backups.prune_backups()
	local backup_root = luanti_backups.world_path .. "/backups"
	...
	if #backups > retention then
		...
		for i = 1, to_remove do
			minetest.log("action", "[luanti_backups] Pruning old backup: " .. backups[i].path)
